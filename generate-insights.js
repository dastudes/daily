...
function generateHTML(date, updatedStr, narratives, factSheet, voiceName, noGames = false) {
    const dateLabel = new Date(date + 'T12:00:00').toLocaleDateString('en-US', {
        weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
    });
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .no-games { font-size: 1.05rem; color: #92400e; font-style: italic; }
        .factsheet-pre {
...
<body>
    ...
    <main class="main-content">
        ...
        ${noGames ? '<p class="no-games">No games yesterday.</p>' : factSheetPanel + panels}
    </main>
    <footer>${noGames ? 'Generated' : 'Generated by Claude AI'} &bull; ${updatedStr}</footer>
    <script>
...
async function main() {
    ...
    // No completed games: skip Claude entirely (nothing to summarize or fact-check),
    // clear the snippets, and remove the brief body so generate-rss.js skips the feed item.
    if (boxscore.games.length === 0) {
        console.log(`No completed games on ${date} — skipping Claude calls, brief, and RSS item`);
        fs.writeFileSync('insights.html', generateHTML(date, updatedStr, [], null, null, true));
        fs.writeFileSync('whats-to-know-snippet.html', '');
        fs.writeFileSync('boxscores-brief-snippet.html', '');
        fs.rmSync('boxscores-brief-body.html', { force: true });
        console.log('Generated no-games insights.html and cleared snippets');
        return;
    }
